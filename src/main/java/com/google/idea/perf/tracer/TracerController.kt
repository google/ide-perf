...
class TracerController(
    ...
    fun startDataRefreshLoop() {
        ...
        executor.scheduleWithFixedDelay(::refreshCallTreeData, 0, REFRESH_DELAY_MS, MILLISECONDS)
    }
    ...
    // TODO: No need to have this code here, we can put it in TracerPanel and use Swing timers.
    private fun refreshCallTreeData() {
        invokeLater {
            // In order to measure tracer UI overhead we need to measure the time elapsed
            // while updating the tree model, *and* also the time it takes to run all the
            // invokeLater calls generated by the update.
            val start = System.nanoTime()
            var invokeLaterStart = 0L
            invokeLater { invokeLaterStart = System.nanoTime() }
            ...
            view.setTracingOverhead(tracingOverhead)

            val elapsedDirectly = System.nanoTime() - start
            invokeLater { // Wait for invokeLater tasks triggered by the tree update.
                val elapsedIndirectly = System.nanoTime() - invokeLaterStart
                view.addUiOverhead(elapsedDirectly + elapsedIndirectly)
            }
        }

        // Backpressure for the data refresh loop.
        invokeAndWaitIfNeeded {}
        invokeAndWaitIfNeeded {} // Handles second layer of invokeLater calls.
    }
